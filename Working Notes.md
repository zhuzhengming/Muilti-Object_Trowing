...
Traditional robotic throwing focus primarily on single-object task with gripper. or throwing multiple object in the same target position, which are not sufficient for the complexity introduced by multi-object manipulation. This research aims to define a methodology based on previous work that enables robotic systems to handle multiple objects and multiple targets in one motion planning.

Timeline, ……

The system framework is here, first we get the target position and robot base position via optitrack. Here Velocity hedgehog and BRT are tow offline generated data structure. Then searching from them to get a lot of feasible solutions, but we need filter some solution according to my own rules and choose a final throwing state. Next step is trajectory generation considering position, velocity, acceleration, jerk limitation and tracking by impedance control in joint space. And control frequency is 200 hz.

Completed work, ……

Some notations are defined here,  A is robot base, B is target position, E is end effector. r is the distance from A to B in x0y plane, z is the height of B. phi is the yaw throwing angle, gamma is pitch throwing angel. The problem can be formulated as to find configurations within joint constraint and inside Backwards reachable tube(BRT). BRT is a continuous set, which derives from object fly dynamics under gravity and can make sure the landing state is inside target landing set. Another offline generated data structure is a dictionary with four keys(height z, distance to base, and throwing angle gamma phi), the corresponding value is maximum end effector velocity. It is generated by solving a linear program optimization problem.  And here is some visualization of BRT and velocity hedgehog. At height equals 0.15m, distance equals 0.7m, the distrubution of maximum velocity is like this. After selecting a suitable solution, I applied ruckig to generate optimized trajectory when consider the position, velocity, acceleration and jerk limitation. And then tracking the trajectory by impedance controller in joint space. 

Next part is consider grasping and release uncertainty using allegro hand. To reduce these uncertainty, …… 

Next part is some experiment result. here is release delay under predefined grasping posture, the delay is around 40-50 ms. Then here is a video about reactive throwing on real robot, I changed the target position randomly. It can alway generate a good trajectory and throw in the box as long as inside the reachable space. And here is analysis of trajectory, because accurate tracking can ensure reach the desire throwing state and landing into target position.  For iiwa 7, as you can see, the position and velocity error of joint 1 is large. I found that the possible reason it has little available torque to acceleration. In torquecontrl mode, the maximum torque is bounded by 80 N·m whereas the booksheet says it can reach 175 N·m, and about 75 N·m is used to compensate gravity of robot. And from the comparison of trajectory in Cartesain space, here is also large error in z direction caused by the joint 1. And I made a object like this to collection real flying data. For a fixed target position, the landing MAE and STD is small. This is a quantitative analysis table, as we can see, the release state offset is small after considering release delay and grasping posture. And the main error is form the trajectory tracking. We can say robot can throw into target box under tolerance 0.15 m. 
