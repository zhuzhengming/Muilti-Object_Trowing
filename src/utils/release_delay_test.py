...
class FTDataRecorder:
    ...
    def plot_exist_data(self):
        ...
        home_times = self.event_data[self.event_data[:, 1] == 'home'][:, 0].astype(float)
        ...
        if len(home_times) > 0:
            ...
            nearest_indices = [np.argmin(np.abs(times - t)) for t in home_times]
            nearest_times = times[nearest_indices]
